...
export default function WatchesPage() {
  ...
  useEffect(() => {
    const fetchProducts = () => {
      try {
        // Get watch products - filter by category and keyword matching
        const watchProducts = allProducts.filter(p => 
          p.category === 'Fashion' && 
          (p.name.toLowerCase().includes('watch') || 
           p.name.toLowerCase().includes('wristwatch') || 
           p.name.toLowerCase().includes('chronograph') ||
           p.name.toLowerCase().includes('diver') ||
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-3/4">
            <div className="flex items-center justify-between mb-8">
              <h2 className="h2">All Watches</h2>
            ...
            {products.length > 0 ? (
              ...
              <div className="text-center py-16">
                <p className="text-lg text-[#555555]">No watches available at the moment.</p>
              </div>
            )}
          </div>
        </div>
