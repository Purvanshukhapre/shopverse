    ...
    >
      ...
      <Link href={`/product/${product.slug}`} className="relative aspect-[4/5] overflow-hidden bg-[#F8F8F8] flex-shrink-0 group">
        <Image
          ...
          fill
          className={cn(
            "object-contain p-12 transition-transform duration-1000 ease-[0.19, 1, 0.22, 1]", // Normalized padding
        ...
        <div className="absolute top-5 left-5 flex flex-col gap-2">
          ...
          <AnimatePresence>
            {isHovered && product.rating >= 4.8 && (
              <motion.div 
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                className="bg-[#111111] text-white text-[9px] font-black px-3.5 py-2 rounded-full shadow-xl shadow-black/20 tracking-[0.2em] uppercase flex items-center gap-2"
              >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-full group-hover:translate-y-0 transition-transform duration-700 ease-[0.19, 1, 0.22, 1]">
          <button
            ...
            className={cn(
              "btn-premium w-full !h-14 !rounded-2xl !text-[10px] !tracking-[0.25em]",
              isAdding 
          ...
          <AnimatePresence>
            {isHovered && (
              <motion.div 
                ...
                className="flex items-center gap-1.5 bg-[#F8F8F8] px-2.5 py-1 rounded-lg border border-gray-100"
              >
